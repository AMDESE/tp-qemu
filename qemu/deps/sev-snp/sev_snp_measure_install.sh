...
case "$OS" in
    ubuntu|debian)
        ...
        check_status "Failed to install python3 / pip / venv / git"
        ;;
    ...
    sles|suse|opensuse-leap|opensuse-tumbleweed)
        ...
        ;;
    *)
        echo "Warning: unsupported OS '$OS'; assuming git + python3 (with venv) are already installed."
        ;;
esac
...
$SUDO python3 -m venv --clear "$VENV_DIR"
check_status "Failed to create virtualenv at $VENV_DIR (is python3-venv installed?)"

$SUDO "$VENV_PY" -m pip install --upgrade pip >/dev/null 2>&1 || true

echo "Installing Python runtime dependencies..."
REQS="$INSTALL_DIR/requirements.txt"
if [ -f "$REQS" ]; then
    $SUDO "$VENV_PY" -m pip install -r "$REQS"
    check_status "Failed to install Python dependencies into $VENV_DIR"
else
...
exec "$VENV_DIR/bin/python" "$INSTALL_DIR/sev-snp-measure.py" "\$@"
EOF
